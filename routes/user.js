...
router.post("/register", async (req, res) => {
    ...
    .then((pg) => {
        if(!pg) return res.status(403).json({success: false, msg: 'Invalid credentials provided!'})
        body['pages'] = pg;
        ...
        User.findOne({ enroll: req.body.enroll }, (err, _user) => {
            ...
            user.save((err, doc) => {
                if (err) return res.json({ success: false, err, msg:'Yes iam' });
                console.log('doc: ',doc)
    ...
    .catch((err) => {
        return res.status(500).json({success: false, err, msg: 'Some Server Error Please try Again Later!'})
    })
