...
exports.main = async (enroll, pass) => {
  ...
  if ($("#ProfileInfo_lblUsername").text() == enroll) { 
    ...
    if ($("#courseId").length) {
      ...
      $("select#courseId option").each(function (index) {
        console.log()
        values.push({key: $(this).val(), value: $(this).text().trim().split('\n')});
      });
  ...
  else if ($("#pageContent > div.container-fluid > div.row > div > div:nth-child(1) > div > strong").text() === 'Error!') {
    browser.close();
    return false;
  }
  await browser.close();
...
exports.getAuthenticate = async (body) => {
    ...
    console.log($("#pageContent > div.container-fluid > div.row > div > div:nth-child(1) > div > strong").text() === 'Error!')
    if ($("#pageContent > div.container-fluid > div.row > div > div:nth-child(1) > div > strong").text() === 'Error!') {
      browser.close();
      return false;
    }
