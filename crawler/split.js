...
const getOneByOneAssignmentDeadlines = ( val ) => {
    $ = cheerio.load(val);
    let row = $('body > div > div.content-wrapper > section.content >\
    div > div > div > div.box-body.table-responsive.no-padding > table tbody').children('tr');
...
//   console.log(cheerios.html($));
  ...
  const getRowData = (ass) => {
      let futureAssignment = [], passedAssignment = [];
